...
const SuperUserDashboard = ({ profile, setProfile, setView, setTask }) => {
	...
	const columns = [
		...
		},
		{
			...
			render: (text, record) => (
				<Space size="middle">
					<Popconfirm
						title={`Sure to remove ${record.name}?`}
						onConfirm={() => {
	...
	const addPatientForm = [
		{
			...
			rules: [
				{
					required: true,
					message: "Please select a patient id to add!",
				},
			],
			input: (
				<Select
	...
	const onPatientFormFinish = (values) => {
		setConfirmLoading(true);
		sendPatientRequest(values)
			.then((res) => {
				const tempProfile = { ...profile };
				tempProfile.pendingRequests = res.usersRequested;
	...
	useEffect(() => {
		if (profile.pendingRequests) {
			setPendingReviewData(
				profile.pendingRequests.map((user, index) => {
					return { patientId: user, key: index };
				})
			);
		}
	}, [profile.pendingRequests]);

	const pendingReviewColumn = [
		{
			title: "Patient Id",
			dataIndex: "patientId",
		},
		{
			...
			render: (record) => {
				return (
					<Popconfirm
						title={`Sure to remove request?`}
						onConfirm={() => {
							cancelPatientRequest(record)
								.then((res) => {
									showNotification(res.message);
									populateAddPatientOption();
									let tempProfile = { ...profile };
									tempProfile.pendingRequests =
										tempProfile.pendingRequests.filter(
	...
	return (
		<Spin spinning={videoOptions.length === 0 || confirmLoading}>
			...
			/>
			<Typography.Title level={2}>Pending Request</Typography.Title>
			<Divider />
			<Table
				columns={pendingReviewColumn}
